...
fileprivate var log:OSLog = {
    let log:OSLog = OSLog(subsystem: "xDrip", category: "loop-client")
    return log
}()
...
/// for use in NSLog
fileprivate let tracePrefix = "xDrip-NSLog"

/// filename prefix for tracing
fileprivate let traceFileNamePrefix = "xdriptrace"

/// filename prefix for tracing. (UI would not be be developed to send the file via e-mail. Not sure if it's the same file as the file generated by xDrip4iOS)
...
func trace(_ message: StaticString, category: String, _ args: CVarArg...) {
    ...
    if traceFileName ==  nil {
        traceFileName = getDocumentsDirectory().appendingPathComponent(traceFileNamePrefix + ".0.log")
    }
    ...
    let timeStamp = dateFormatNSLog.string(from: Date())
        ...
        NSLog("%@", tracePrefix + " " + timeStamp + " " + category + " " + actualMessage)
    ...
    do {
        ...
        NSLog("%@", tracePrefix + " " + dateFormatNSLog.string(from: Date()) + " write trace to file failed")
...
fileprivate func rotateTraceFiles() {
    ...
    let lastFile = getDocumentsDirectory().appendingPathComponent(traceFileNamePrefix + ".0.log")
    ...
    for indexFrom0ToMax in 0...1 {
        ...
        let file = getDocumentsDirectory().appendingPathComponent(traceFileNamePrefix + "." + index.description + ".log")
        let newFile = getDocumentsDirectory().appendingPathComponent(traceFileNamePrefix + "." + (index + 1).description + ".log")
...
class Trace {
    ...
    static func getTraceFilesInData() -> ([Data], [String]) {
        ...
        for index in 0..<3 {
            ...
            let filename = traceFileNamePrefix + "." + index.description + ".log"
